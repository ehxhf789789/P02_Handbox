...
async function executeNodeReal(
  ...
  // ===== KISTI ScienceON 검색 노드 - Python API =====
  // 노드 타입별 서비스 자동 매핑
  const kistiNodeTypeMap: Record<string, string> = {
    'kisti-articles': 'ARTI',
    'kisti-patents': 'PATENT',
    'kisti-reports': 'REPORT',
    'kisti-trends': 'ATT',
  }
  const isKistiNode = nodeType === 'kisti-search' || nodeType === 'api-kisti' ||
    nodeType in kistiNodeTypeMap || label.includes('KISTI') || label.includes('ScienceON')

  if (isKistiNode) {
    // 서비스 타입: ARTI(논문), PATENT(특허), REPORT(보고서), ATT(동향)
    // 노드 타입에서 자동 감지, 없으면 config에서, 기본값 ARTI
    const target = kistiNodeTypeMap[nodeType] || config.target || config.service_type || 'ARTI'
    const query = config.query || config.search_query || predecessorOutputs[0]?.query || '건설신기술'
    const searchField = config.search_field || 'BI'  // BI: 기본색인
    const rowCount = config.row_count || config.limit || 10
    const curPage = config.cur_page || config.page || 1

    if (!query) {
      return {
        error: '검색어가 설정되지 않았습니다',
        status: '검색어 필요',
        results: [],
        total_count: 0
      }
    ...
    try {
      ...
      if (result.success) {
        ...
        const serviceLabel = serviceLabels[target.toUpperCase()] || target
        ...
        return {
          error: result.error || 'KISTI 검색 실패',
          status: 'KISTI API 오류',
          results: [],
          total_count: 0
        }
      ...
      return {
        error: `KISTI API 실패: ${String(error)}`,
        status: 'KISTI API 서버 연결 필요',
        results: [],
        total_count: 0
      }
    }
